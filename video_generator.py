...
def create_video_for_sentence_ffmpeg(audio_path, image_folder, output_path, target_resolution="1080x1920", zoom_out=True):
    ...
    print(f"Found {len(images)} images in {image_folder}.")  # Print how many images were found
    ...
    print(f"Selected image: {selected_image}")  # Print which image was selected

    # Construct the ffmpeg command with scaling and zoom-out for 9:16 aspect ratio
    if zoom_out:
    ...
    command = [
        ...
        '-loop', '1',  # Loop the image to match the audio duration
        '-i', image_path,  # Input image
        '-i', audio_path,  # Input audio
        '-vf', zoom_filter,  # Apply the zoom filter
...
def generate_and_concatenate_videos_ffmpeg(audio_base_path, images_base_path, sentences, output_folder, final_output_path, target_resolution="1080x1920", zoom_out=True):
    ...
    for key, value in tqdm(sentences.items(), desc="Generating Videos for Sentences"):
        ...
        try:
            create_video_for_sentence_ffmpeg(audio_file, image_folder, output_video, target_resolution, zoom_out)
...
if __name__ == "__main__":
    ...
    generate_and_concatenate_videos_ffmpeg(audio_folder, images_output_folder, story_data.get("sentences", {}), video_output_folder, final_video_output, target_resolution="1080x1920", zoom_out=True)
